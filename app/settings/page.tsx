...
const FULL_RECOVERY_KEY = 'afterimage:settings-recovery:v1';

function readAfterimageData() {
  const data: Record<string, unknown> = {};
  for (let i = 0; i < localStorage.length; i += 1) {
    const key = localStorage.key(i);
    if (!key || !key.startsWith(AFTERIMAGE_PREFIX) || key === FULL_RECOVERY_KEY) continue;
...
export default function SettingsPage() {
  ...
  function refreshInventory() {
    ...
    setCanUndo(Boolean(localStorage.getItem(FULL_RECOVERY_KEY)));
  }
  ...
  function exportData() {
    ...
    downloadText('afterimage-local-data.json', JSON.stringify(payload, null, 2));
    setMessage('All Afterimage local data was exported. Nothing was uploaded.');
  }

  async function exportEncrypted() {
    if (backupPassword.length < 8) { setMessage('Use a backup password with at least 8 characters.'); return; }
    ...
    try {
      ...
      downloadText('afterimage-secure-backup.json', encrypted);
      setBackupPassword('');
      setBackupConfirm('');
      setMessage('Encrypted backup created locally. Your password was not stored.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to create encrypted backup.');
  ...
  async function importEncrypted(file: File) {
    ...
    try {
      ...
      if (!Array.isArray(current)) throw new Error('Current memory store is invalid.');
      ...
      const prefs = backup.preferences as { signalPreferences?: unknown; localData?: Record<string, unknown> } | undefined;
      const localData = prefs?.localData;
      if (localData && typeof localData === 'object') {
        for (const [key, value] of Object.entries(localData)) {
          if (key.startsWith(AFTERIMAGE_PREFIX) && key !== FULL_RECOVERY_KEY && key !== MEMORY_KEY) {
            localStorage.setItem(key, typeof value === 'string' ? value : JSON.stringify(value));
          }
        }
      }
      localStorage.setItem(MEMORY_KEY, JSON.stringify([...existing.values()].slice(0, 1000)));
      if (prefs?.signalPreferences !== undefined) localStorage.setItem(PREF_KEY, JSON.stringify(prefs.signalPreferences));
      setImportPassword('');
      setMessage(`Encrypted backup restored and merged. ${backup.memories.length} memories were processed.`);
      refreshInventory();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to import encrypted backup.');
    } finally { setBusy(false); }
  }
  ...
  function clearAll() {
    ...
    const memories = Array.isArray(data[MEMORY_KEY]) ? data[MEMORY_KEY] : [];
    try {
      localStorage.setItem(FULL_RECOVERY_KEY, JSON.stringify({ format: 'afterimage.settings-recovery.v1', createdAt: new Date().toISOString(), data }));
    } catch {}
    ...
    setCanUndo(true);
    setMessage('Local memory cleared. Your previous Afterimage data can be restored with Undo clear.');
    refreshInventory();
  }

  function undoClear() {
    try {
      const raw = localStorage.getItem(FULL_RECOVERY_KEY);
      if (raw) {
        const snapshot = JSON.parse(raw) as { data?: Record<string, unknown> };
        if (snapshot.data && typeof snapshot.data === 'object') {
          for (const [key, value] of Object.entries(snapshot.data)) {
            if (key.startsWith(AFTERIMAGE_PREFIX) && key !== FULL_RECOVERY_KEY) {
              localStorage.setItem(key, typeof value === 'string' ? value : JSON.stringify(value));
            }
          }
          localStorage.removeItem(FULL_RECOVERY_KEY);
          setCanUndo(false);
          setMessage('All recoverable Afterimage data was restored.');
          refreshInventory();
          return;
        }
      }
      ...
      setCanUndo(false);
      setMessage(restored ? `Restored ${restored.length} local memor${restored.length === 1 ? 'y' : 'ies'}.` : 'No recoverable local snapshot is available.');
      refreshInventory();
    } catch { setMessage('Unable to restore the recovery snapshot.'); }
  }
  ...
  function eraseEverything() {
    ...
    setConfirmClear(false);
    setCanUndo(false);
    setMessage(`Permanently erased ${removed} Afterimage local storage entries. No recovery snapshot was kept.`);
    refreshInventory();
  }
  ...
  return <main className="settings-page"><div className="settings-shell">
    ...
    <section className="settings-card"><div className="settings-card-icon"><KeyRound size={18}/></div><div><h2>Encrypted backup</h2><p>Create an AES-256-GCM backup in your browser. The password never leaves this device.</p><div className="settings-inline-fields"><input type="password" value={backupPassword} onChange={e=>setBackupPassword(e.target.value)} placeholder="Backup password (8+ chars)" autoComplete="new-password"/><input type="password" value={backupConfirm} onChange={e=>setBackupConfirm(e.target.value)} placeholder="Confirm password" autoComplete="new-password"/></div></div><button disabled={busy||backupPassword.length<8||!backupConfirm} onClick={exportEncrypted}><Download size={14}/> Secure export</button></section>
    <section className="settings-card"><div className="settings-card-icon"><Upload size={18}/></div><div><h2>Restore encrypted backup</h2><p>Import and merge a secure backup. A recovery snapshot is created before changes.</p><div className="settings-inline-fields"><input type="password" value={importPassword} onChange={e=>setImportPassword(e.target.value)} placeholder="Backup password" autoComplete="off"/><input type="file" accept="application/json,.json" disabled={busy||!importPassword} onChange={e=>{const file=e.target.files?.[0]; if(file) void importEncrypted(file); e.currentTarget.value='';}}/></div></div><span className="settings-status"><ShieldCheck size={13}/> Local</span></section>
    <section className="settings-card"><div className="settings-card-icon"><Undo2 size={18}/></div><div><h2>Recovery</h2><p>Destructive library clearing creates a full local recovery snapshot before removal.</p></div>{canUndo&&<button onClick={undoClear}><Undo2 size={14}/> Undo clear</button>}</section>
